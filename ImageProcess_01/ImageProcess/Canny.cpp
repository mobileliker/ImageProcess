// Canny.cpp: implementation of the CCanny class.
//
//////////////////////////////////////////////////////////////////////
...
void CCanny::CreatGauss(double sigma, double **pdKernel, int *pnWidowSize)
{
...
//用高斯滤波器平滑原图像
void CCanny::GaussianSmooth(SIZE sz, LPBYTE pGray, LPBYTE pResult, double sigma)
{
 LONG x, y;
 LONG i;
...
// 方向导数,求梯度
void CCanny::Grad(SIZE sz, LPBYTE pGray,int *pGradX, int *pGradY, int *pMag)
{
 LONG y,x;
...
//非最大抑制
void CCanny::NonmaxSuppress(int *pMag, int *pGradX, int *pGradY, SIZE sz, LPBYTE pNSRst)
{
 LONG y,x;
 int nPos;
...
// 统计pMag的直方图，判定阈值
void CCanny::EstimateThreshold(int *pMag, SIZE sz, int *pThrHigh, int *pThrLow, LPBYTE pGray,
...
//利用函数寻找边界起点
void CCanny::Hysteresis(int *pMag, SIZE sz, double dRatLow, double dRatHigh, LPBYTE pResult)
{
 LONG y,x;
...
void CCanny::TraceEdge(int y, int x, int nThrLow, LPBYTE pResult, int *pMag, SIZE sz)
{
 //对8邻域像素进行查询
...
// Canny算子
void CCanny::Canny(LPBYTE pGray, SIZE sz, double sigma, double dRatLow,
